...
class ImportDriverTypeFixer extends React.Component {
    ...
    async addHistory(driver, license_plate,cylinders, number_cylinder, idDriver, sign, fromId) {
        ...
        const user = await createHistoryAPI(driver, license_plate, cylinders, Constants.IMPORT_TYPE, this.state.ListUserSubmitID, '', number_cylinder, fromId, '', '', '', '', '', typeExportCylinder, '', idDriver, sign);
    ...
    async submit(event) {
        ...
        await this.addHistory(nameDriver, data.license_plate,cylinders, data.number_cylinder, data.idDriver, sign, this.props.product_parse[0].histories[0].from.id);
        return;
    }
    handeload = () =>{
